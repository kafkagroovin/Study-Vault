...
async function ai(messages, system) {
  const r = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      system: system || "You are StudyBot, a witty friendly AI tutor. Be concise, helpful, encouraging.",
...
const isThisMonth = s => { const d=new Date(s), n=new Date(); return d.getMonth()===n.getMonth()&&d.getFullYear()===n.getFullYear(); };
...
/* ─── Theme system ────────────────────────────────────────────────────────── */
const THEMES = [
  { id:"violet",   name:"Neon Violet", desc:"Default dark purple",  acc:"#7c3aed", acc2:"#a78bfa", glow:"rgba(124,58,237,0.4)",  bg:"#070912", orb1:"rgba(124,58,237,0.22)", orb2:"rgba(167,139,250,0.15)" },
  { id:"rose",     name:"Rose Gold",   desc:"Pink luxury vibes",    acc:"#e11d48", acc2:"#fb7185", glow:"rgba(225,29,72,0.4)",   bg:"#0f0608", orb1:"rgba(225,29,72,0.22)",   orb2:"rgba(251,113,133,0.15)" },
  { id:"ocean",    name:"Ocean Deep",  desc:"Cool blue tones",      acc:"#0284c7", acc2:"#38bdf8", glow:"rgba(2,132,199,0.4)",   bg:"#040d14", orb1:"rgba(2,132,199,0.22)",   orb2:"rgba(56,189,248,0.15)"  },
  { id:"emerald",  name:"Emerald",     desc:"Fresh green energy",   acc:"#059669", acc2:"#34d399", glow:"rgba(5,150,105,0.4)",   bg:"#040f0b", orb1:"rgba(5,150,105,0.22)",   orb2:"rgba(52,211,153,0.15)"  },
  { id:"sunset",   name:"Sunset",      desc:"Warm orange glow",     acc:"#ea580c", acc2:"#fb923c", glow:"rgba(234,88,12,0.4)",   bg:"#100805", orb1:"rgba(234,88,12,0.22)",   orb2:"rgba(251,146,60,0.15)"  },
  { id:"midnight", name:"Midnight",    desc:"Pure dark minimal",    acc:"#6366f1", acc2:"#818cf8", glow:"rgba(99,102,241,0.4)",  bg:"#05050f", orb1:"rgba(99,102,241,0.22)",  orb2:"rgba(129,140,248,0.15)" },
];
...
function Toasts() {
  ...
  return (
    <div style={{ position:"fixed", bottom:NAV+14, left:"50%", transform:"translateX(-50%)", zIndex:9900, display:"flex", flexDirection:"column", gap:6, pointerEvents:"none" }}>
      {list.map(x => <div key={x.id} style={{ background:"#131729", border:"1px solid "+B2, borderRadius:50, padding:"10px 20px", fontSize:13, fontWeight:600, color:TX, whiteSpace:"nowrap", boxShadow:"0 8px 28px rgba(0,0,0,0.5)", animation:"toastIn 0.4s cubic-bezier(0.34,1.56,0.64,1) forwards" }}>{x.msg}</div>)}
    </div>
  );
}
...
function Btn({ children, onClick, variant, size, disabled, style }) {
  ...
  const vr = v==="ghost"  ? {background:G2,color:TX,border:"1px solid "+B1}
           ...
           :                {background:"linear-gradient(135deg,"+t.acc+","+t.acc+"cc)",color:"#fff",border:"none",boxShadow:"0 4px 18px "+t.glow};
  ...
  return (
    <div style={{ marginBottom:14 }}>
      ...
      <input style={{ width:"100%",background:G1,border:"1px solid "+B1,borderRadius:12,padding:"12px 16px",color:TX,fontSize:14,outline:"none",...style }}
        onFocus={e=>{e.target.style.borderColor=t.acc;e.target.style.boxShadow="0 0 0 3px "+t.glow;}}
        onBlur={e =>{e.target.style.borderColor=B1; e.target.style.boxShadow="none";}} {...p}/>
    </div>
  );
}

function Spin() { const t=getT(); return <div style={{ width:18,height:18,border:"2px solid "+B2,borderTopColor:t.acc2,borderRadius:"50%",animation:"spin 0.7s linear infinite",flexShrink:0 }}/>; }
...
function Sheet({ open, onClose, title, children }) {
  ...
  return (
    <div onClick={e=>e.target===e.currentTarget&&onClose()} style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",backdropFilter:"blur(8px)",zIndex:600,display:"flex",alignItems:"flex-end",justifyContent:"center",animation:"bkIn 0.25s ease" }}>
      <div style={{ background:"#0d1021",border:"1px solid "+B2,borderRadius:"24px 24px 0 0",padding:"24px 20px 40px",width:"100%",maxWidth:500,animation:"slideUp 0.35s cubic-bezier(0.34,1.56,0.64,1)",maxHeight:"88vh",overflowY:"auto" }}>
...
/* ═══════════════════════════════════════════════════════════════════════════ */
/*  AUTH — Login / Register / Magic Link / Guest                               */
/* ═══════════════════════════════════════════════════════════════════════════ */
function Auth({ onAuth }) {
  const [tab,   setTab]   = useState("login");   // login | register | magic
  const [step,  setStep]  = useState("form");    // form | otp | onboard
  ...
  /* Login */
  const doLogin = () => {
    if (!email.trim()||!pass) { toast("Fill in all fields 📝"); return; }
    const accounts = LS.get("sv_accounts", {});
    const acct = accounts[email.trim().toLowerCase()];
    if (!acct) { toast("No account found — register first!"); return; }
    if (acct.hash !== btoa(pass)) { toast("Wrong password ❌"); return; }
    onAuth({ method:"login", email:email.trim(), name:acct.name, type:acct.type, avatar:acct.avatar||"🎓" });
  };

  /* Register step 1 */
  const doRegister = () => {
    if (!email.trim()||!pass||!pass2) { toast("Fill in all fields 📝"); return; }
    if (pass.length < 6) { toast("Password must be 6+ characters"); return; }
    if (pass !== pass2) { toast("Passwords don't match ❌"); return; }
    const accounts = LS.get("sv_accounts", {});
    if (accounts[email.trim().toLowerCase()]) { toast("Email already registered!"); return; }
    setPUser({ method:"login", email:email.trim() });
    setStep("onboard");
  };

  /* Register step 2 — save & auth */
  const finishRegister = () => {
    if (!name.trim()) { toast("Tell us your name 😊"); return; }
    const accounts = LS.get("sv_accounts", {});
    accounts[email.trim().toLowerCase()] = { hash:btoa(pass), name:name.trim(), type:stype, avatar:"🎓" };
    LS.set("sv_accounts", accounts);
  ...
  /* Guest */
  const goGuest = () => onAuth({ method:"guest", email:"", name:"Guest", type:"Student", avatar:"👤" });

  const eyeBtn = (show, toggle) => (
    <button type="button" onClick={toggle} style={{ position:"absolute",right:14,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",color:TX2,cursor:"pointer",fontSize:15,lineHeight:1 }}>
      {show ? "🙈" : "👁"}
    </button>
  );
  ...
  return (
    <div style={{ minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 20px",position:"relative",overflow:"hidden",background:t.bg }}>
      <div style={{ position:"absolute",top:"-25%",left:"-20%",width:500,height:500,borderRadius:"50%",background:"radial-gradient(circle,"+t.orb1+" 0%,transparent 70%)",animation:"orb1 7s ease-in-out infinite alternate",pointerEvents:"none" }}/>
      <div style={{ position:"absolute",bottom:"-20%",right:"-15%",width:420,height:420,borderRadius:"50%",background:"radial-gradient(circle,"+t.orb2+" 0%,transparent 70%)",animation:"orb2 5s ease-in-out infinite alternate",pointerEvents:"none" }}/>
      ...
      <div style={{ position:"relative",zIndex:1,width:"100%",maxWidth:400,animation:"fadeUp 0.6s ease" }}>
        ...
        <div style={{ textAlign:"center",marginBottom:28 }}>
          <div style={{ fontFamily:"'Syne',sans-serif",fontSize:46,fontWeight:800,lineHeight:1 }}>
            Study<span style={{ color:t.acc2,textShadow:"0 0 40px "+t.acc }}>Vault</span>
          </div>
        ...
        <div style={{ background:"rgba(255,255,255,0.05)",border:"1px solid "+B2,borderRadius:24,padding:26,backdropFilter:"blur(24px)" }}>
          ...
          {step === "onboard" && (
            <div>
              <button onClick={()=>setStep("form")} style={{ background:"none",border:"none",color:TX2,fontSize:13,marginBottom:14,cursor:"pointer" }}>← Back</button>
              <div style={{ textAlign:"center",fontSize:38,marginBottom:12 }}>👋</div>
              <Field label="Your name" placeholder="e.g. Alex" value={name} onChange={e=>setName(e.target.value)}/>
              ...
              <div style={{ display:"flex",flexWrap:"wrap",gap:8,marginBottom:20 }}>
                {TYPES.map(([em,lb])=>(
                  <button key={lb} onClick={()=>setStype(lb)} style={{ padding:"8px 14px",borderRadius:50,fontSize:12,fontWeight:600,border:"1px solid "+(stype===lb?t.acc:B1),background:stype===lb?"rgba(124,58,237,0.2)":G1,color:stype===lb?t.acc2:TX2,cursor:"pointer" }}>{em} {lb}</button>
                ))}
              </div>
              <Btn style={{ width:"100%" }} onClick={tab==="register" ? finishRegister : finishMagic}>Let's Go 🚀</Btn>
            </div>
          )}
          ...
          {step === "otp" && (
            <div>
              <button onClick={()=>setStep("form")} style={{ background:"none",border:"none",color:TX2,fontSize:13,marginBottom:14,cursor:"pointer" }}>← Back</button>
              <p style={{ fontSize:13,color:TX2,marginBottom:16,lineHeight:1.6 }}>Enter the code sent to <b style={{color:TX}}>{email}</b>:</p>
          ...
          {step === "form" && (
            <div>
              {/* Tab bar */}
              <div style={{ display:"flex",background:G2,borderRadius:14,padding:4,marginBottom:22,border:"1px solid "+B1 }}>
                {[["login","🔑 Log In"],["register","✨ Register"],["magic","✉️ Magic"]].map(([tb,lb])=>(
                  <button key={tb} onClick={()=>{setTab(tb);setEmail("");setPass("");setPass2("");}}
                    style={{ flex:1,padding:"9px 0",borderRadius:10,fontSize:12,fontWeight:700,border:"none",cursor:"pointer",transition:"all 0.2s",background:tab===tb?t.acc:"none",color:tab===tb?"#fff":TX2,boxShadow:tab===tb?"0 2px 12px "+t.glow:"none" }}>{lb}</button>
                ))}
              </div>
              ...
              {tab === "login" && (
                <div>
                  ...
                  <div style={{ marginBottom:16 }}>
                    <div style={{ fontSize:11,fontWeight:700,color:TX2,textTransform:"uppercase",letterSpacing:"0.8px",marginBottom:6 }}>Password</div>
                    <div style={{ position:"relative" }}>
                      <input type={showP?"text":"password"} placeholder="Your password" value={pass} onChange={e=>setPass(e.target.value)}
                        onKeyDown={e=>e.key==="Enter"&&doLogin()}
                        style={{ width:"100%",background:G1,border:"1px solid "+B1,borderRadius:12,padding:"12px 42px 12px 16px",color:TX,fontSize:14,outline:"none" }}
                        onFocus={e=>{e.target.style.borderColor=t.acc;e.target.style.boxShadow="0 0 0 3px "+t.glow;}}
                        onBlur={e =>{e.target.style.borderColor=B1; e.target.style.boxShadow="none";}}/>
                      {eyeBtn(showP, ()=>setShowP(s=>!s))}
                    </div>
                  </div>
                  <Btn style={{ width:"100%",marginBottom:14 }} onClick={doLogin}>Log In →</Btn>
                  <div style={{ textAlign:"center",fontSize:12,color:TX2 }}>
                    No account?{" "}
                    <button onClick={()=>setTab("register")} style={{ background:"none",border:"none",color:t.acc2,cursor:"pointer",fontWeight:700,fontSize:12 }}>Register here</button>
                  </div>
                </div>
              )}
              ...
              {tab === "register" && (
                <div>
                  ...
                  <div style={{ marginBottom:14 }}>
                    <div style={{ fontSize:11,fontWeight:700,color:TX2,textTransform:"uppercase",letterSpacing:"0.8px",marginBottom:6 }}>Password</div>
                    <div style={{ position:"relative" }}>
                      <input type={showP?"text":"password"} placeholder="Min. 6 characters" value={pass} onChange={e=>setPass(e.target.value)}
                        style={{ width:"100%",background:G1,border:"1px solid "+B1,borderRadius:12,padding:"12px 42px 12px 16px",color:TX,fontSize:14,outline:"none" }}
                        onFocus={e=>{e.target.style.borderColor=t.acc;e.target.style.boxShadow="0 0 0 3px "+t.glow;}}
                        onBlur={e =>{e.target.style.borderColor=B1; e.target.style.boxShadow="none";}}/>
                      {eyeBtn(showP, ()=>setShowP(s=>!s))}
                  ...
                  <div style={{ marginBottom:18 }}>
                    <div style={{ fontSize:11,fontWeight:700,color:TX2,textTransform:"uppercase",letterSpacing:"0.8px",marginBottom:6 }}>Confirm Password</div>
                    <div style={{ position:"relative" }}>
                      <input type={showP2?"text":"password"} placeholder="Repeat password" value={pass2} onChange={e=>setPass2(e.target.value)}
                        style={{ width:"100%",background:G1,border:"1px solid "+B1,borderRadius:12,padding:"12px 42px 12px 16px",color:TX,fontSize:14,outline:"none" }}
                        onFocus={e=>{e.target.style.borderColor=t.acc;e.target.style.boxShadow="0 0 0 3px "+t.glow;}}
                        onBlur={e =>{e.target.style.borderColor=B1; e.target.style.boxShadow="none";}}/>
                      {eyeBtn(showP2, ()=>setShowP2(s=>!s))}
                    </div>
                  </div>
                  <Btn style={{ width:"100%",marginBottom:14 }} onClick={doRegister}>Create Account ✨</Btn>
                  <div style={{ textAlign:"center",fontSize:12,color:TX2 }}>
                    Have an account?{" "}
                    <button onClick={()=>setTab("login")} style={{ background:"none",border:"none",color:t.acc2,cursor:"pointer",fontWeight:700,fontSize:12 }}>Log in</button>
                  </div>
                </div>
              )}
              ...
              {tab === "magic" && (
                <div>
                  <p style={{ fontSize:13,color:TX2,marginBottom:16,lineHeight:1.6 }}>Get a one-time code — no password needed!</p>
                  <Field label="Email" type="email" placeholder="you@email.com" value={email} onChange={e=>setEmail(e.target.value)}/>
                  <Btn style={{ width:"100%",marginBottom:12 }} onClick={sendOtp}>Send Code ✨</Btn>
                </div>
              )}

              {/* Divider */}
              <div style={{ display:"flex",alignItems:"center",gap:12,margin:"16px 0" }}>
                <div style={{ flex:1,height:1,background:B1 }}/><span style={{ fontSize:11,color:TX3,fontWeight:600 }}>OR</span><div style={{ flex:1,height:1,background:B1 }}/>
              </div>

              {/* Guest */}
              <button onClick={goGuest} style={{ display:"flex",alignItems:"center",gap:12,width:"100%",padding:14,background:G1,border:"1px solid "+B1,borderRadius:14,cursor:"pointer",color:TX,transition:"all 0.2s" }}
                onMouseEnter={e=>{e.currentTarget.style.borderColor=YEL;e.currentTarget.style.background="rgba(251,191,36,0.08)";}}
                onMouseLeave={e=>{e.currentTarget.style.borderColor=B1; e.currentTarget.style.background=G1;}}>
                <div style={{ width:38,height:38,borderRadius:10,background:"rgba(251,191,36,0.15)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0 }}>⚡</div>
                <div>
                  <div style={{ fontSize:13,fontWeight:700 }}>Continue as Guest</div>
                  <div style={{ fontSize:11,color:TX2,marginTop:1 }}>Instant access, no signup</div>
                </div>
              </button>
            </div>
          )}
        </div>
        <div style={{ textAlign:"center",marginTop:18,fontSize:11,color:TX3 }}>By signing up you agree to have a great study session 📚</div>
      </div>
    </div>
  );
}
...
function Home({ user, sessions, transactions, notes, streak, budget }) {
  const [tip, setTip] = useState(""); const [tipLoad, setTipLoad] = useState(true);
  const t = getT();
  ...
  useEffect(()=>{
    setTipLoad(true);
    ai("Give one surprising actionable study tip. 1-2 sentences max. No preamble.")
      .then(r=>{setTip(r);setTipLoad(false);})
      .catch(()=>{setTip("Teach concepts out loud — if you can explain it simply, you truly understand it.");setTipLoad(false);});
  },[]);
  ...
  return (
    <div style={{ paddingBottom:16 }}>
      <div style={{ padding:"22px 18px 6px",display:"flex",alignItems:"flex-start",justifyContent:"space-between" }}>
        ...
        <div style={{ width:46,height:46,borderRadius:"50%",background:"linear-gradient(135deg,"+t.acc+","+t.acc+"88)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,border:"2px solid "+B2 }}>{user.avatar}</div>
      </div>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,padding:"14px 14px 0" }}>
        ...
        <Card style={{ gridColumn:"1/-1",background:t.acc+"12",borderColor:t.acc+"38" }}>
          <div style={{ fontSize:11,fontWeight:700,color:t.acc2,textTransform:"uppercase",letterSpacing:0.8,marginBottom:10 }}>✦ AI Tip of the Day</div>
          {tipLoad?<div style={{ display:"flex",gap:10,alignItems:"center",color:TX2,fontSize:13 }}><Spin/> Thinking…</div>
...
function Study({ sessions, onAdd }) {
  ...
  const startTimer=()=>{setRun(true);setMood(false);iv.current=setInterval(()=>{setRem(r=>{if(mode==="free")return r+1;if(r<=1){clearInterval(iv.current);setRun(false);done(tot);return 0;}return r-1;});},1000);};
  ...
  const analyze=async()=>{if(sessions.length<2){toast("Log at least 2 sessions first!");return;}setAiLoad(true);setAiRes("");const data=sessions.slice(-14).map(s=>new Date(s.date).toLocaleDateString("en",{weekday:"short"})+": "+s.subject+" "+Math.round(s.duration/60)+"min mood "+s.mood+"/5").join("; ");const r=await ai("Analyze these study sessions: give 3 short insights with emojis. "+data);setAiRes(r);setAiLoad(false);};
    ...
    <div style={{ paddingBottom:16 }}>
      ...
      <div style={{ display:"flex",gap:6,margin:"0 16px 14px",padding:4,background:G1,borderRadius:50,border:"1px solid "+B1 }}>
        {[["pomo","🍅 Pomodoro"],["sht","⚡ Short"],["free","⏱ Free"]].map(([m,lb])=>(
          <button key={m} onClick={()=>switchMode(m)} style={{ flex:1,padding:"8px 0",borderRadius:50,fontSize:12,fontWeight:700,border:"none",cursor:"pointer",background:mode===m?t.acc:"none",color:mode===m?"#fff":TX2,boxShadow:mode===m?"0 2px 12px "+t.glow:"none",transition:"all 0.2s" }}>{lb}</button>
        ))}
      </div>
      ...
      <div style={{ display:"flex",flexDirection:"column",alignItems:"center",padding:"4px 0 14px" }}>
        ...
        <div style={{ display:"flex",gap:14,alignItems:"center" }}>
          ...
          <button onClick={run?pause:startTimer} style={{ width:70,height:70,borderRadius:"50%",background:"linear-gradient(135deg,"+t.acc+","+t.acc+"aa)",border:"none",color:"#fff",fontSize:28,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",boxShadow:"0 6px 28px "+t.glow }}>{run?"⏸":"▶"}</button>
      ...
      <div style={{ padding:"0 16px 10px" }}>
        <Btn variant="ghost" style={{ width:"100%",marginBottom:10 }} onClick={analyze} disabled={aiLoad}>{aiLoad?<><Spin/> Analyzing…</>:"✦ Analyze My Patterns"}</Btn>
        {aiRes&&<Card style={{ background:t.acc+"12",borderColor:t.acc+"38",fontSize:14,lineHeight:1.75,whiteSpace:"pre-wrap" }}>{aiRes}</Card>}
      </div>
...
function Money({ transactions, budget, onAddTxn, onDelTxn, onSetBudget }) {
  const [addOpen,setAddOpen]=useState(false),[budOpen,setBudOpen]=useState(false),[roast,setRoast]=useState(""),[roastLoad,setRoastLoad]=useState(false),[form,setForm]=useState({type:"expense",amount:"",desc:"",cat:"Food",emoji:"🍕"}),[budF,setBudF]=useState({monthly:budget.monthly||"",currency:budget.currency||"$"});
  const CATS=[["🍕","Food"],["📚","Books"],["🚌","Transport"],["🎮","Entertainment"],["🏠","Housing"],["💊","Health"],["🛒","Shopping"],["☕","Coffee"],["💰","Other"]];
  const t=getT(), c=budget.currency||"$";
  ...
  const doRoast=async()=>{const exp=transactions.filter(t=>t.type==="expense"&&isThisMonth(t.date));if(exp.length<2){toast("Add more expenses first!");return;}setRoastLoad(true);setRoast("");const data=exp.map(t=>t.cat+": "+c+t.amount).join(", ");const r=await ai("You're a hilarious student financial advisor. Roast this spending in 3 funny sentences then give 2 real tips. Budget: "+c+(budget.monthly||"?")+" Data: "+data);setRoast(r);setRoastLoad(false);};
    ...
    <div style={{ paddingBottom:16 }}>
      ...
      <div style={{ padding:"0 14px 14px" }}>
        <Card>
          ...
          ):<div style={{ fontSize:13,color:TX2 }}>Tap <b style={{ color:t.acc2 }}>Edit</b> to set your monthly budget</div>}
        </Card>
      </div>
      {catRows.length>0&&(
        <div style={{ padding:"0 14px 14px" }}>
      ...
      <Sheet open={addOpen} onClose={()=>setAddOpen(false)} title="Add Transaction">
        ...
        <Field label="Description" placeholder="e.g. Lunch" value={form.desc} onChange={e=>setForm(f=>({...f,desc:e.target.value}))}/>
        ...
        <div style={{ display:"flex",flexWrap:"wrap",gap:6,marginBottom:18 }}>
          {CATS.map(([em,nm])=><button key={nm} onClick={()=>setForm(f=>({...f,cat:nm,emoji:em}))} style={{ padding:"7px 12px",borderRadius:50,fontSize:12,fontWeight:600,border:"1px solid "+(form.cat===nm?t.acc:B1),background:form.cat===nm?t.acc+"33":G1,color:form.cat===nm?t.acc2:TX2,cursor:"pointer" }}>{em} {nm}</button>)}
        </div>
      ...
      <Sheet open={budOpen} onClose={()=>setBudOpen(false)} title="Set Budget">
        <div style={{ marginBottom:14 }}>
          ...
          <select value={budF.currency} onChange={e=>setBudF(f=>({...f,currency:e.target.value}))} style={{ width:"100%",background:G1,border:"1px solid "+B1,borderRadius:12,padding:"12px 16px",color:TX,fontSize:14,outline:"none" }}>
            {["$","€","£","₹","¥","₦","R"].map(cv=><option key={cv} value={cv} style={{ background:"#0d1021" }}>{cv}</option>)}
          </select>
        </div>
        ...
        <Btn style={{ width:"100%" }} onClick={()=>{onSetBudget({monthly:parseFloat(budF.monthly)||0,currency:budF.currency});setBudOpen(false);toast("Budget saved! 💰");}}>Save Budget</Btn>
      </Sheet>
    </div>
  );
}
...
function Notes({ notes, onSave }) {
  ...
  if (editing) return <NoteEditor note={editing} onSave={n=>{onSave(n);setEditing(null);}} onClose={()=>setEditing(null)}/>;
  return (
    <div style={{ paddingBottom:16 }}>
...
function NoteEditor({ note, onSave, onClose }) {
  ...
  const doAi=async action=>{
    ...
    try {
      if(action==="summarize"){const r=await ai("Summarize this note in 5 bullet points, each starting with •:\n\n"+txt);setAiPanel({content:r});}
      else if(action==="explain"){const r=await ai("Explain simply for a 16-year-old using analogies:\n\n"+txt.slice(0,1500));setAiPanel({content:r});}
      else if(action==="expand"){const r=await ai("Expand this note with more detail and examples:\n\n"+txt.slice(0,1500));if(edRef.current)edRef.current.innerHTML+="<br><hr style='border:none;border-top:1px solid rgba(255,255,255,0.1);margin:14px 0'><strong style='color:#a78bfa'>✦ Expanded</strong><br><br>"+r.replace(/\n/g,"<br>");autoSave();toast("Expanded ✅");}
      else if(action==="grammar"){const r=await ai("Fix grammar/spelling. Return ONLY corrected text:\n\n"+txt);if(edRef.current)edRef.current.innerHTML=r.replace(/\n/g,"<br>");autoSave();toast("Grammar fixed ✓");}
      else if(action==="quiz"){const raw=await ai('Create 5 MCQs. Return ONLY JSON: [{"q":"...","options":["A","B","C","D"],"answer":0}]\n\n'+txt.slice(0,2000));const qs=JSON.parse(raw.replace(/```json|```/g,"").trim());setQuiz({qs,i:0,score:0,sel:null,done:false,answered:false});}
      else if(action==="flashcards"){const raw=await ai('Create 6 flashcard pairs. Return ONLY JSON: [{"front":"term","back":"definition"}]\n\n'+txt.slice(0,2000));const cards=JSON.parse(raw.replace(/```json|```/g,"").trim());setFc(cards);setFcI(0);setFcFlip(false);}
    } catch { toast("AI error — try again"); }
    setAiLoad(false);
  };
  ...
  const nextQ=()=>{const ni=quiz.i+1;if(ni>=quiz.qs.length){const s=quiz.score+(quiz.sel===quiz.qs[quiz.i].answer?1:0);setQuiz(q=>({...q,done:true,score:s}));if(s>=4)confetti();}else setQuiz(q=>({...q,i:ni,sel:null,answered:false}));};
  ...
  return (
    <div style={{ position:"fixed",inset:0,background:t.bg,zIndex:300,display:"flex",flexDirection:"column",overflow:"hidden" }}>
      ...
      <div style={{ display:"flex",gap:5,padding:"8px 12px",borderBottom:"1px solid "+B1,overflowX:"auto",scrollbarWidth:"none",flexShrink:0,background:"#0d1021" }}>
      ...
      {(aiLoad||aiPanel||quiz||fc)&&(
        <div style={{ borderTop:"1px solid "+B1,padding:16,maxHeight:260,overflowY:"auto",background:"#0d1021",flexShrink:0 }}>
          ...
          {aiPanel&&!quiz&&!fc&&(
            <div>
              ...
              <div style={{ display:"flex",gap:8 }}>
                <Btn size="sm" onClick={()=>{if(edRef.current)edRef.current.innerHTML+="<br><hr style='border:none;border-top:1px solid rgba(255,255,255,0.1);margin:14px 0'><strong style='color:#a78bfa'>✦ AI Result</strong><br><br>"+aiPanel.content.replace(/\n/g,"<br>");setAiPanel(null);autoSave();toast("Inserted ✅");}}>Insert</Btn>
          ...
          {fc&&(
            <div>
              ...
              <div style={{ display:"flex",gap:10,marginTop:10 }}>
                ...
                <Btn size="sm" onClick={()=>{if(fcI<fc.length-1){setFcI(f=>f+1);setFcFlip(false);}else{setFc(null);confetti();toast("Complete! 🎉");}}}>{fcI<fc.length-1?"Next →":"Done 🎉"}</Btn>
              </div>
            </div>
          )}
        </div>
...
function AiChat({ user }) {
  ...
  const send=async text=>{const msg=(text||input).trim();if(!msg)return;setInput("");setLoad(true);const newMsgs=[...msgs,{role:"user",content:msg}];setMsgs(newMsgs);const history=newMsgs.map(m=>({role:m.role==="assistant"?"assistant":"user",content:m.content}));const reply=await ai(history,"You are StudyBot, a friendly smart AI tutor. Be encouraging, clear, occasionally witty. Help with any academic topic.").catch(()=>"Something went wrong. Try again! 😅");setMsgs(m=>[...m,{role:"assistant",content:reply}]);setLoad(false);};
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",height:"calc(100vh - "+NAV+"px)",overflow:"hidden" }}>
      ...
      <div style={{ display:"flex",gap:8,padding:"10px 12px",overflowX:"auto",scrollbarWidth:"none",flexShrink:0 }}>
        {CHIPS.map(c=>(
          <button key={c} onClick={()=>send(c)} style={{ whiteSpace:"nowrap",padding:"8px 14px",borderRadius:50,background:G1,border:"1px solid "+B1,color:TX2,fontSize:12,fontWeight:600,cursor:"pointer",flexShrink:0 }}
            onMouseEnter={e=>{e.target.style.background=t.acc+"25";e.target.style.borderColor=t.acc;e.target.style.color=t.acc2;}}
            onMouseLeave={e=>{e.target.style.background=G1;e.target.style.borderColor=B1;e.target.style.color=TX2;}}>{c}</button>
        ))}
      </div>
      <div style={{ flex:1,overflowY:"auto",padding:"10px 14px",display:"flex",flexDirection:"column",gap:12 }}>
        {msgs.map((m,i)=>(
          <div key={i} style={{ display:"flex",gap:10,flexDirection:m.role==="user"?"row-reverse":"row" }}>
            ...
            <div style={{ maxWidth:"80%",padding:"12px 16px",borderRadius:m.role==="assistant"?"4px 18px 18px 18px":"18px 4px 18px 18px",fontSize:14,lineHeight:1.7,background:m.role==="assistant"?G2:"linear-gradient(135deg,"+t.acc+","+t.acc+"aa)",border:m.role==="assistant"?"1px solid "+B1:"none",color:"#fff",whiteSpace:"pre-wrap" }}>{m.content}</div>
          </div>
        ))}
        {load&&(
      ...
      <div style={{ padding:"12px 14px",borderTop:"1px solid "+B1,display:"flex",gap:10,alignItems:"flex-end",flexShrink:0,background:t.bg }}>
        ...
        <button onClick={()=>send()} disabled={load||!input.trim()} style={{ width:46,height:46,borderRadius:"50%",background:"linear-gradient(135deg,"+t.acc+","+t.acc+"aa)",border:"none",color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:"0 4px 16px "+t.glow,cursor:load||!input.trim()?"not-allowed":"pointer",opacity:load||!input.trim()?0.5:1 }}>
...
/*  SETTINGS                                                                   */
/* ═══════════════════════════════════════════════════════════════════════════ */
function Settings({ user, onUpdateUser, onClearData, budget, onThemeChange }) {
  ...
  return (
    <div style={{ paddingBottom:24 }}>
      ...
      <div style={{ padding:"0 14px 16px" }}>
        <Card>
          <div style={{ textAlign:"center",marginBottom:16 }}>
            ...
            <div style={{ fontSize:12,color:TX2,marginTop:4 }}>{user.type} · {user.method==="guest"?"Guest":user.method==="login"?"Logged in":"Magic link"}</div>
          </div>
      ...
      {/* 🎨 Theme Templates */}
      <div style={{ padding:"0 14px 16px" }}>
        <div style={{ fontSize:11,fontWeight:700,color:TX2,textTransform:"uppercase",letterSpacing:0.8,marginBottom:12 }}>🎨 Theme Templates</div>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10 }}>
          {THEMES.map(th=>(
            <div key={th.id} onClick={()=>{ onThemeChange(th); toast(th.name+" applied! 🎨"); }}
              style={{ padding:"14px",background:th.id===t.id?th.acc+"18":G1,border:"2px solid "+(th.id===t.id?th.acc:B1),borderRadius:16,cursor:"pointer",transition:"all 0.2s",position:"relative" }}
              onMouseEnter={e=>{e.currentTarget.style.borderColor=th.acc;e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.boxShadow="0 8px 24px rgba(0,0,0,0.3)";}}
              onMouseLeave={e=>{e.currentTarget.style.borderColor=th.id===t.id?th.acc:B1;e.currentTarget.style.transform="";e.currentTarget.style.boxShadow="";}}>
      ...
      <div style={{ padding:"0 14px 14px" }}>
        ...
        {[
          ...
          {icon:"🗑",title:"Clear All Data",sub:"Permanently delete everything",action:()=>{if(confirm("Delete all data? Cannot be undone."))onClearData();},danger:true},
        ].map(item=>(
      ...
      <div style={{ textAlign:"center",padding:"6px 24px 4px",fontSize:13,color:TX2 }}>
        Made with 💜 by <span style={{ color:t.acc2,fontWeight:800,fontFamily:"'Syne',sans-serif" }}>Porte Boshi</span>
      </div>
...
export default function App() {
  ...
  const [theme,        setTheme]        = useState(()=>{ const saved=LS.get("sv_theme","violet"); return THEMES.find(t=>t.id===saved)||THEMES[0]; });
  const [, forceRender] = useState(0);

  // Keep module-level theme in sync
  _theme = theme;

  const applyTheme = th => { _theme=th; setTheme(th); LS.set("sv_theme",th.id); forceRender(n=>n+1); };
  ...
  /* Dynamic CSS that reacts to theme */
  const dynamicCss = BASE_CSS + "html,body{background:"+theme.bg+"!important;}";
  ...
  const VIEW = {
    ...
    money:    <Money    transactions={transactions} budget={budget} onAddTxn={tx=>saveTransactions([...transactions,tx])} onDelTxn={id=>saveTransactions(transactions.filter(tx=>tx.id!==id))} onSetBudget={saveBudget}/>,
    notes:    <Notes    notes={notes} onSave={saveNote}/>,
    ai:       <AiChat   user={user}/>,
    settings: <Settings user={user} onUpdateUser={u=>{setUser(u);LS.set("sv_user",u);}} onClearData={clearData} budget={budget} onThemeChange={applyTheme}/>,
  };
  ...
  return (
    <>
      <style>{dynamicCss}</style>
      <Toasts/>
      <div style={{ background:theme.bg, minHeight:"100vh", paddingBottom:NAV }}>
        <div key={tab} style={{ animation:"fadeUp 0.35s ease" }}>{VIEW[tab]}</div>
        <nav style={{ position:"fixed",bottom:0,left:0,right:0,height:NAV,background:theme.bg+"ee",backdropFilter:"blur(24px)",borderTop:"1px solid "+B1,display:"flex",alignItems:"center",justifyContent:"space-around",zIndex:200,paddingBottom:4 }}>
          {NAV_ITEMS.map(item=>(
            <button key={item.id} onClick={()=>setTab(item.id)} style={{ flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3,padding:"8px 2px",background:"none",border:"none",color:tab===item.id?theme.acc2:TX3,cursor:"pointer",transition:"all 0.25s",borderRadius:12 }}>
              <div style={{ transform:tab===item.id?"scale(1.15)":"scale(1)",transition:"all 0.25s",filter:tab===item.id?"drop-shadow(0 0 6px "+theme.acc+")":"none" }}>{item.icon}</div>
