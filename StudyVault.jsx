...
/* ─── AI (Runs entirely in browser - no API key needed!) ─────────────────── */
let generator = null;
  ...
  try {
    ...
    if (!generator) {
      generator = await pipeline('text2text-generation', 'Xenova/LaMini-Flan-T5-783M', {
        quantized: true, // Smaller model for browser
      });
    }
    ...
    <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"40px 24px", position:"relative", overflow:"hidden", background:BG }}>
      ...
      <div style={{ position:"relative", zIndex:1, width:"100%", maxWidth:380, animation:"fadeUp 0.6s ease" }}>
        ...
        <div style={{ background:"rgba(255,255,255,0.05)", border:"1px solid " + B2, borderRadius:24, padding:26, backdropFilter:"blur(24px)" }}>
          {step === "main" && (
            <div>
              <p style={{ fontSize:13, color:TX2, marginBottom:16, lineHeight:1.65 }}>Welcome! Let's get started 👇</p>
              <button onClick={goGuest}
                ...
                <div style={{ width:42, height:42, borderRadius:12, background:"rgba(251,191,36,0.15)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:22, flexShrink:0 }}>⚡</div>
                <div>
                  <div style={{ fontSize:14, fontWeight:700 }}>Continue as Guest</div>
                  <div style={{ fontSize:12, color:TX2, marginTop:2 }}>Start instantly, no signup required</div>
                </div>
              </button>
            </div>
          )}
          ...
          {step === "onboard" && (
            <div>
              ...
              <div style={{ fontSize:11, fontWeight:700, color:TX2, textTransform:"uppercase", letterSpacing:"0.8px", marginBottom:10 }}>Choose your avatar</div>
              <div style={{ display:"flex", flexWrap:"wrap", gap:8, justifyContent:"center", marginBottom:16 }}>
                {avatars.map(a => (
                  <span key={a} onClick={() => setAvatar(a)} style={{ fontSize:26, cursor:"pointer", opacity:avatar === a ? 1 : 0.4, transform:avatar === a ? "scale(1.2)" : "scale(1)", transition:"all 0.2s", display:"inline-block" }}>{a}</span>
                ))}
              </div>
...
function Home({ user, sessions, transactions, notes, streak, budget }) {
  ...
  useEffect(() => {
    setTipLoad(true);
    ai("Give one surprising actionable study tip for a student. 1-2 sentences max. No preamble.")
      .then(t => { setTip(t); setTipLoad(false); })
      .catch(() => { 
        setTip("Teach concepts out loud — if you can explain it simply, you truly understand it."); 
        setTipLoad(false); 
      });
  }, []);
    ...
    <div style={{ paddingBottom:16 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, padding:"14px 14px 0" }}>
        ...
        <Card style={{ gridColumn:"1/-1", background:"rgba(124,58,237,0.07)", borderColor:"rgba(124,58,237,0.22)" }}>
          <div style={{ fontSize:11, fontWeight:700, color:ACC2, textTransform:"uppercase", letterSpacing:0.8, marginBottom:10 }}>✦ AI Tip of the Day</div>
          {tipLoad
            ? <div style={{ display:"flex", gap:10, alignItems:"center", color:TX2, fontSize:13 }}><Spin /> Thinking…</div>
...
function Study({ sessions, onAdd }) {
  ...
  const analyze = async () => {
    ...
    const data = sessions.slice(-14).map(s => 
      new Date(s.date).toLocaleDateString("en",{weekday:"short"}) + ": " + 
      s.subject + " " + Math.round(s.duration/60) + "min mood " + (s.mood || 3) + "/5"
    ).join("; ");
    const r = await ai("Analyze these student study sessions. Give 3 short specific insights with emojis about patterns and tips. Sessions: " + data);
...
function Money({ transactions, budget, onAddTxn, onDelTxn, onSetBudget }) {
  ...
  const doRoast = async () => {
    ...
    const r = await ai("You're a hilarious student financial advisor. Roast this spending in 3 funny sentences then give 2 real money-saving tips. Budget: " + c + (budget.monthly || "?") + ". Data: " + data);
...
function NoteEditor({ note, onSave, onClose }) {
  ...
  const doAi = async action => {
    ...
    if (txt.length < 15) { toast("Write some content first! 📝"); return; }
    setAiLoad(true); setAiPanel(null); setQuiz(null); setFc(null);
    try {
      if (action === "summarize") {
        const r = await ai("Summarize this student note in 5 bullet points, each starting with •:\n\n" + txt);
        setAiPanel({ type:"text", content:r });
      } else if (action === "explain") {
        const r = await ai("Explain the concepts in this note super simply for a 16-year-old using analogies:\n\n" + txt.slice(0, 1500));
        setAiPanel({ type:"text", content:r });
      } else if (action === "expand") {
        const r = await ai("Expand this student note with more detail and examples:\n\n" + txt.slice(0, 1500));
        if (edRef.current) edRef.current.innerHTML += "<br><hr style='border:none;border-top:1px solid rgba(255,255,255,0.1);margin:14px 0'><strong style='color:#a78bfa'>✦ Expanded</strong><br><br>" + r.replace(/\n/g, "<br>");
        autoSave(); toast("Expanded ✅");
      } else if (action === "grammar") {
        const r = await ai("Fix all grammar, spelling, punctuation. Return ONLY the corrected text:\n\n" + txt);
        if (edRef.current) edRef.current.innerHTML = r.replace(/\n/g, "<br>");
        autoSave(); toast("Grammar fixed ✓");
      } else if (action === "quiz") {
        const raw = await ai("Create 5 multiple-choice questions from this content. Return ONLY a JSON array: [{\"q\":\"...\",\"options\":[\"A\",\"B\",\"C\",\"D\"],\"answer\":0}]. answer is 0-indexed.\n\n" + txt.slice(0, 2000));
        const qs = JSON.parse(raw.replace(/```json|```/g, "").trim());
        setQuiz({ qs, i:0, score:0, sel:null, done:false, answered:false });
      } else if (action === "flashcards") {
        const raw = await ai("Create 6 flashcard pairs from this note. Return ONLY JSON: [{\"front\":\"term\",\"back\":\"definition\"}]\n\n" + txt.slice(0, 2000));
...
function AiChat({ user }) {
  const [msgs, setMsgs]     = useState([{ role:"assistant", content:"Hey! I'm StudyBot 👋 Ask me anything — concepts, essays, math, study plans. I'm here to help!" }]);
  ...
  const send = async text => {
    ...
    const reply = await ai(history, "You are StudyBot, a friendly smart AI tutor for students. Be encouraging, clear, occasionally witty. Help with any academic topic. Use line breaks for readability.").catch(() => "Something went wrong. Try again! 😅");
    ...
    <div style={{ paddingBottom:20 }}>
      ...
      <div style={{ padding:"0 14px 14px" }}>
        ...
        {[
          ...
          { icon:"🗑", title:"Clear All Data",  sub:"Permanently delete everything", action:() => { if (confirm("Delete all data? This cannot be undone.")) onClearData(); }, danger:true },
        ].map(item => (
      ...
      <div style={{ textAlign:"center", padding:"8px 24px", color:TX3, fontSize:12 }}>StudyVault v2.0 ✨ AI runs 100% in your browser</div>
    </div>
  );
}
...
export default function App() {
  ...
  const addSession = s => {
    ...
    if (ns.lastDate !== today) { 
      ns.current = ns.lastDate === yesterday ? ns.current + 1 : 1; 
      ns.lastDate = today; 
    }
